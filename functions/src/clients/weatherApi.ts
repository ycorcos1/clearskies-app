...
const getWeatherApiKey = (): string => {
  const apiKey = functions.config().weather?.api_key as string | undefined;

  if (!apiKey) {
    throw new Error(
      "Weather API key is not configured. Run `firebase functions:config:set weather.api_key=\"YOUR_KEY\"`."
    );
  }
